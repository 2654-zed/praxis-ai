...
function showResults(data) {
  ...
  if (stack.length === 0) {
    ...
    stack.forEach((entry) => {
      ...
      html += `
        <div class="stack-card" style="${role === 'primary' ? 'box-shadow:inset 4px 0 0 #667eea, 0 2px 16px rgba(0,0,0,0.22), 0 0 0 1px rgba(255,255,255,0.03);' : ''}">
          ...
          <div class="confidence-bar">
            <div class="confidence-fill" style="width:${fitScore}%;background:linear-gradient(90deg,#667eea,#764ba2);"></div>
          </div>
      ...
      if (priceParts.length > 0 || skillLevel) {
        ...
        if (skillLevel) html += `<span class="tag" style="background:rgba(124,106,239,0.10);color:#a99cf7;padding:0.25rem 0.75rem;border-radius:8px;font-size:0.78rem;">${cap(skillLevel)}</span>`;
        html += `</div>`;
      }
...
function openCompare(toolName, event) {
  ...
  modal.innerHTML = `
    ...
    <div class="modal-content">
      ...
      <h3>Compare with…</h3>
      <p style="color:rgba(255,255,255,0.45);font-size:0.9rem;margin-bottom:1rem;">Select a tool to compare against <strong style="color:#e8eaf6;">${toolName}</strong></p>
...
async function runCompare(toolA, toolB) {
  ...
  try {
    ...
    html += `<h4 style="margin-bottom:0.5rem;color:#e8eaf6;">${toolA} vs ${toolB}</h4>`;    
    ...
    html += `<tr style="border-bottom:1px solid rgba(255,255,255,0.04);"><th></th><th style="padding:0.5rem;color:#7c6aef;">${toolA}</th><th style="padding:0.5rem;color:#50e3c2;">${toolB}</th></tr>`;
    ...
    resultEl.innerHTML = `<p style="color:#f06060;margin-top:1rem;">Comparison failed — ${e.message}</p>`;
  }
}
